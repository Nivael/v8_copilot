# v8 delivery todo index

| ID | Item | Status | Blocked by |
| ---: | --- | --- | --- |
| 1 | P6A 管理人实体与节点模式 | pending | — |
| 2 | P6B 重整前偿债压力与市场重定价地图 | in_progress | — |
| 3 | P6C 重整方案价值重估 | pending | 2 |
| 4 | P7 每日公告、异常量价与研究优先级 | in_progress | — |
| 5 | P8 研究漏斗、分阶段成交情景参考与前哨链 | in_progress | — |

See `V8_P6_INSIGHTS_PRD.md`, `V8_P6B_PRD.md` and
`V8_P7_DAILY_INTELLIGENCE_PRD.md` and `V8_P8_RESEARCH_FUNNEL_PRD.md` for
architecture, validation and release gates.
