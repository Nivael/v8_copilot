...
2. 构建/更新五轨事件图；正文 LLM 只有在 owner 明确允许公开公告正文发送到 OpenAI 后运行；
...
owner 已明确批准“把公开上市公司公告正文发送给 OpenAI API”后，才可运行：
...
该命令以 `announcement_id + content digest + model + prompt version` 缓存；规则/模型不一致、
低置信、引用无法在正文定位都不会升级为事实。
...
`p8_publish.py` 要求八类 run 全部对齐同一日。正文核证、旧股东精确权益或组合观察天数不足是
