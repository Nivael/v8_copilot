...
2. 把“个股涨跌幅”升级为“个股绝对收益 + ST 板块收益 + 大盘收益 + 三组相对收益”。
...
flowchart LR
    ...
    D --> F
    G["Tushare 000985.CSI"] --> H["market context store"]
    F --> H
...
这些差值是描述性的百分点差，不包装成 alpha 或因果因子暴露。三条序列必须使用同一交易日边界；任一基准缺端点时返回 gap，不做自然日插值。
