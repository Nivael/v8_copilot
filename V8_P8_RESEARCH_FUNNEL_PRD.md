...
状态：architecture frozen；P8-0 数据门待执行；修订 P6B / P7 的研究边界
日期：2026-09-04
...
- 优先使用公司自身已核证方案形成的成功/失败情景；跨公司参考只能作情景敏感性，不得自动
  替代公司特定值；
- 成功、失败值来自 §3.3 各自独立分布，不因当前量价形态选择样本；
...
同一股票命中两条以上通道时跨通道晋级，但只占一个展示位；随后按证据质量、最近新事实日期、
命中通道数和 symbol 确定性排序。`provisional`、`unknown`、退市风险和负面分支不通过扣分偷偷
