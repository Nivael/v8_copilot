...
@app.post("/api/v1/answers", response_model=ResearchResponse)
def answers(request: ResearchRequest) -> ResearchResponse:
    try:
...
def _ndjson_stream(request: ResearchRequest) -> Iterator[str]:
    ...
    try:
        ...
        for event in stream_events(request_with_id, response):
...
def stock_dossier(
    ...
    try:
        return build_stock_dossier(symbol)
