...
function EvidencePackView({packId,run}:{packId:string;run:ResearchRun}) {
  ...
  return <div className="pack-audit">
    ...
    {open&&<div className="pack-body">
      ...
      {record&&<>
        <div className="pack-summary"><span>查询计划：{record.payload.query_plan_id}</span><span>Digest：{record.pack_digest.slice(0,16)}…</span><span>Manifest：{String(record.payload.freshness_manifest.manifest_id||record.payload.freshness_manifest.status||'未记录')}</span><span>{record.payload.rows.length} 行数据库证据</span><span>{record.payload.lens_invocations.length} 条 Lens</span></div>
