/** 内部代号 → 人话展示映射（D-008：不向最终用户暴露英文 debug 代号）。 */
...
export function show(value: unknown): string {
  ...
  if (text.startsWith('announcement:')) return `公告编号 ${text.split(':')[1]}`
  if (text.startsWith('v5_lifecycle_jsonl_seed')) return 'ST 生命周期历史快照'
  if (text === 'v5_2026_title_audit_lower_bound') return '2026 公告标题审计下界'
  return inlineLabels
    .reduce((result, [key, label]) => result.replaceAll(key, label), text)
    .replaceAll('C17 wording', '历史股价行为样本措辞边界')
    .replaceAll('wording', '措辞边界')
}
